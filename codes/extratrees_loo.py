...
Original file is located at
    https://colab.research.google.com/drive/11SpX_Vjt5HH3rbxY7Mr8bYEkBLdYnffD
"""
...
def plot_comparison(phase_name, y_exp, y_nist, y_model, save_path=None, name=None):
    ...
    plt.scatter(y_exp, y_model, color='red', label='ExtraTrees', marker = '+', s=40, linewidth=1)
...
# ***** DECISION TREE *****
DT_file = check_and_get_filename(current_directory, "DecisionTree_Physical-State-Determination_ExtraTrees", "log")
...
ExtraTrees_directory = os.path.join(property_directory, "ExtraTrees-cvloo")
os.makedirs(ExtraTrees_directory, exist_ok=True)
ExtraTrees_file = check_and_get_filename(ExtraTrees_directory, "ExtraTrees-cvloo", "log")
...
# Grid Search for Hyperparameter Tuning for liquid state with ExtraTrees
print("Tuning hyperparameters for liquid state with ExtraTrees...")
...
save_to_file_and_print(f"\nBest Hyperparameters for liquid state for ExtraTree: {grid_search_liquid.best_params_}.\n", ExtraTrees_file)

model_liquid = grid_search_liquid.best_estimator_
model_liquid.fit(X_liquid_train, y_liquid_train)

joblib.dump(model_liquid, os.path.join(ExtraTrees_directory, 'Model_ExtraTrees_liquid.pkl'))

#model_liquid_path = os.path.join(ExtraTrees_directory, 'Model_ExtraTrees_liquid.pkl')
...
# >>> Train <<<
save_to_file_and_print("\n > Train data <", ExtraTrees_file)
calculate_metrics('liquid', y_liquid_train, y_pred_train_liquid, file=ExtraTrees_file)

# >>> Test <<<
save_to_file_and_print("\n > Test data <", ExtraTrees_file)
calculate_metrics('liquid', y_liquid_test,  y_pred_test_liquid, file=ExtraTrees_file)

plot_overfitting_evaluation(y_liquid_test, y_pred_test_liquid, save_path=ExtraTrees_directory, name="ExtraTrees-overfitting_evaluation-liquid", phase_name="liquid")
...
# Grid Search for Hyperparameter Tuning for vapor state with ExtraTrees
print("Tuning hyperparameters for vapor state with ExtraTrees...")
...
save_to_file_and_print(f"\nBest Hyperparameters for vapor state for ExtraTree: {grid_search_vapor.best_params_}.\n", ExtraTrees_file)

model_vapor = grid_search_vapor.best_estimator_
model_vapor.fit(X_vapor_train, y_vapor_train)

joblib.dump(model_vapor, os.path.join(ExtraTrees_directory, 'Model_ExtraTrees_vapor.pkl'))

#model_vapor_path = os.path.join(ExtraTrees_directory, 'Model_ExtraTrees_vapor.pkl')
...
# >>> Train <<<
save_to_file_and_print("\n > Train data <", ExtraTrees_file)
calculate_metrics('vapor', y_vapor_train, y_pred_train_vapor, file=ExtraTrees_file)

# >>> Test <<<
save_to_file_and_print("\n > Test data <", ExtraTrees_file)
calculate_metrics('vapor', y_vapor_test,  y_pred_test_vapor, file=ExtraTrees_file)

plot_overfitting_evaluation(y_vapor_test, y_pred_test_vapor, save_path=ExtraTrees_directory, name="ExtraTrees-overfitting_evaluation-vapor", phase_name="vapor")
...
# Grid Search for Hyperparameter Tuning for supercritical state with ExtraTrees
print("Tuning hyperparameters for supercritical state with ExtraTrees...")
...
save_to_file_and_print(f"\nBest Hyperparameters for supercritical state for ExtraTree: {grid_search_supercritical.best_params_}.\n", ExtraTrees_file)
...
joblib.dump(model_supercritical, os.path.join(ExtraTrees_directory, 'Model_ExtraTrees_supercritical.pkl'))

#model_supercritical_path = os.path.join(ExtraTrees_directory, 'Model_ExtraTrees_supercritical.pkl')
...
# >>> Train <<<
save_to_file_and_print("\n > Train data <", ExtraTrees_file)
calculate_metrics('supercritical', y_supercritical_train, y_pred_train_supercritical, file=ExtraTrees_file)

# >>> Test <<<
save_to_file_and_print("\n > Test data <", ExtraTrees_file)
calculate_metrics('supercritical', y_supercritical_test,  y_pred_test_supercritical, file=ExtraTrees_file)

plot_overfitting_evaluation(y_supercritical_test, y_pred_test_supercritical, save_path=ExtraTrees_directory, name="ExtraTrees-overfitting_evaluation-supercritical", phase_name="supercritical")


#=====================
# Metrics - All data
#=====================
save_to_file_and_print("\n\nBelow metrics encompass all data.\n", ExtraTrees_file)
...
liquid_score = cross_val_score(model_liquid, X_liquid_Exp, y_liquid_Exp, cv=loo, scoring='neg_root_mean_squared_error')
liquid_cv_mean = np.mean(liquid_score)
liquid_cv_std = np.std(liquid_score)

vapor_score = cross_val_score(model_vapor, X_vapor_Exp, y_vapor_Exp, cv=loo, scoring='neg_root_mean_squared_error')
vapor_cv_mean = np.mean(vapor_score)
vapor_cv_std = np.std(vapor_score)

supercritical_score = cross_val_score(model_supercritical, X_supercritical_Exp, y_supercritical_Exp, cv=loo, scoring='neg_root_mean_squared_error')
supercritical_cv_mean = np.mean(supercritical_score)
supercritical_cv_std = np.std(supercritical_score)


save_to_file_and_print(f"*** Cross-Validation Metrics ***", ExtraTrees_file)
save_to_file_and_print(f"Liquid Phase - RMSE (mean): {-liquid_cv_mean}\nRMSE (std): {liquid_cv_std}\n", ExtraTrees_file)
save_to_file_and_print(f"Vapor Phase - RMSE (mean): {-vapor_cv_mean}\nRMSE (std): {vapor_cv_std}\n", ExtraTrees_file)
save_to_file_and_print(f"Supercritical Phase - RMSE (mean): {-supercritical_cv_mean}\nRMSE (std): {supercritical_cv_std}\n\n\n", ExtraTrees_file)



#liquid
save_to_file_and_print(">>> Liquid <<<\n", ExtraTrees_file)
calculate_metrics("liquid (NIST)", y_liquid_Exp_array, y_liquid_NIST_Expbased_array, file=ExtraTrees_file)
calculate_metrics("liquid (Model)", y_liquid_Exp_array, y_pred_liquid_cv_array, file=ExtraTrees_file)

#vapor
save_to_file_and_print(">>> Vapor <<<\n", ExtraTrees_file)
calculate_metrics("vapor (NIST)", y_vapor_Exp_array, y_vapor_NIST_Expbased_array, file=ExtraTrees_file)
calculate_metrics("vapor (Model)", y_vapor_Exp_array, y_pred_vapor_cv_array, file=ExtraTrees_file)

#supercritical
save_to_file_and_print(">>> Supercritical <<<\n", ExtraTrees_file)
calculate_metrics("supercritical (NIST)", y_supercritical_Exp_array, y_supercritical_NIST_Expbased_array, file=ExtraTrees_file)
calculate_metrics("supercritical (Model)", y_supercritical_Exp_array, y_pred_supercritical_cv_array, file=ExtraTrees_file)

# Liquid phase
plot_comparison("liquid", y_liquid_Exp_array, y_liquid_NIST_Expbased_array, y_pred_liquid_cv_array, save_path=ExtraTrees_directory, name="ModelxNISTxExp-liquid")

# Vapor phase
plot_comparison("vapor",  y_vapor_Exp_array, y_vapor_NIST_Expbased_array, y_pred_vapor_cv_array, save_path=ExtraTrees_directory, name="ModelxNISTxExp-vapor")

# Supercritical phase
plot_comparison("supercritical", y_supercritical_Exp_array, y_supercritical_NIST_Expbased_array, y_pred_supercritical_cv_array, save_path=ExtraTrees_directory, name="ModelxNISTxExp-supercritical")

# NIST X MODEL

save_to_file_and_print("\n\nBelow metrics encompass all data.\n\tComparison NIST x Model\n", ExtraTrees_file)
#remember: calculate_metrics(phase_name, y_true, y_pred, metrics_to_print=None, file=None)
#          def plot_overfitting_evaluation(y_train, y_test, save_path=None, name=None, phase_name=None)
#          def plot_comparison(phase_name, y_true, y_nist, y_pred, save_path=None, name=None)


save_to_file_and_print(">>> NIST x Model <<<\n", ExtraTrees_file)

#liquid
calculate_metrics("Liquid", y_liquid_NIST_Expbased_array, y_pred_liquid_cv_array, file=ExtraTrees_file)

#vapor
calculate_metrics("Vapor", y_vapor_NIST_Expbased_array, y_pred_vapor_cv_array, file=ExtraTrees_file)

#supercritical
calculate_metrics("Supercritical", y_supercritical_NIST_Expbased_array, y_pred_supercritical_cv_array, file=ExtraTrees_file)

# Liquid phase
plot_overfitting_evaluation(y_liquid_NIST_Expbased_array, y_pred_liquid_cv_array, save_path=ExtraTrees_directory, name="ModelxNIST-liquid", phase_name='Liquid')

# Vapor phase
plot_overfitting_evaluation(y_vapor_NIST_Expbased_array, y_pred_vapor_cv_array, save_path=ExtraTrees_directory, name="ModelxNIST-vapor", phase_name='Vapor')

# Supercritical phase
plot_overfitting_evaluation(y_supercritical_NIST_Expbased_array, y_pred_supercritical_cv_array, save_path=ExtraTrees_directory, name="ModelxNIST-Supercritical", phase_name='Supercritical')
...
np.savetxt(os.path.join(ExtraTrees_directory, 'liquid_data.txt'), liquid_data, delimiter='\t', header='Temp(K)\tPressure(MPa)\tExperimental(mW/m*K)\tNIST(mW/m*K)\tModel(mW/m*K)')

# Save vapor data
vapor_data = np.column_stack((X_vapor_Exp, y_vapor_Exp_array, y_vapor_NIST_Expbased_array, y_pred_vapor_cv_array))
np.savetxt(os.path.join(ExtraTrees_directory, 'vapor_data.txt'), vapor_data, delimiter='\t', header='Temp(K)\tPressure(MPa)\tExperimental(mW/m*K)\tNIST(mW/m*K)\tModel(mW/m*K)')
...
np.savetxt(os.path.join(ExtraTrees_directory, 'supercritical_data.txt'), supercritical_data, delimiter='\t', header='Temp(K)\tPressure(MPa)\tExperimental(mW/m*K)\tNIST(mW/m*K)\tModel(mW/m*K)')
