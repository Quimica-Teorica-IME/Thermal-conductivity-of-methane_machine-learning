...
Original file is located at
    https://colab.research.google.com/drive/1KgWB2f76Ema4c7v3-7MiL5ZKs7A_4Fv8
"""
...
AdaboostLasso_directory = os.path.join(property_directory, "AdaboostLasso-cvloo")
os.makedirs(AdaboostLasso_directory, exist_ok=True)
AdaboostLasso_file = check_and_get_filename(AdaboostLasso_directory, "AdaboostLasso-cvloo", "log")
...
save_to_file_and_print(f"\nBest Hyperparameters for vapor state for Adaboost with Decision Lasso: {grid_search_vapor.best_params_}.\n", AdaboostLasso_file)
...
save_to_file_and_print(f"*** Cross-Validation Metrics ***", AdaboostLasso_file)
...
calculate_metrics("liquid (NIST)", y_liquid_Exp_array, y_liquid_NIST_Expbased_array, file=AdaboostLasso_file)
calculate_metrics("liquid (Model)", y_liquid_Exp_array, y_pred_liquid_cv_array, file=AdaboostLasso_file)

#vapor
save_to_file_and_print(">>> Vapor <<<\n", AdaboostLasso_file)
calculate_metrics("vapor (NIST)", y_vapor_Exp_array, y_vapor_NIST_Expbased_array, file=AdaboostLasso_file)
calculate_metrics("vapor (Model)", y_vapor_Exp_array, y_pred_vapor_cv_array, file=AdaboostLasso_file)

#supercritical
save_to_file_and_print(">>> Supercritical <<<\n", AdaboostLasso_file)
calculate_metrics("supercritical (NIST)", y_supercritical_Exp_array, y_supercritical_NIST_Expbased_array, file=AdaboostLasso_file)
calculate_metrics("supercritical (Model)", y_supercritical_Exp_array, y_pred_supercritical_cv_array, file=AdaboostLasso_file)
...
#liquid
save_to_file_and_print(">>> Liquid <<<\n", AdaboostLasso_file)
calculate_metrics("NIST x Model", y_liquid_NIST_Expbased_array, y_pred_liquid_cv_array, file=AdaboostLasso_file)

#vapor
save_to_file_and_print(">>> Vapor <<<\n", AdaboostLasso_file)
calculate_metrics("vapor (NIST)", y_vapor_NIST_Expbased_array, y_pred_vapor_cv_array, file=AdaboostLasso_file)

#supercritical
save_to_file_and_print(">>> Supercritical <<<\n", AdaboostLasso_file)
calculate_metrics("supercritical (NIST)", y_supercritical_NIST_Expbased_array, y_pred_supercritical_cv_array, file=AdaboostLasso_file)
...
np.savetxt(os.path.join(AdaboostLasso_directory, 'liquid_data.txt'), liquid_data, delimiter='\t', header='Temp(K)\tPressure(MPa)\tExperimental(mW/m*K)\tNIST(mW/m*K)\tModel(mW/m*K)')

# Save vapor data
vapor_data = np.column_stack((X_vapor_Exp, y_vapor_Exp_array, y_vapor_NIST_Expbased_array, y_pred_vapor_cv_array))
np.savetxt(os.path.join(AdaboostLasso_directory, 'vapor_data.txt'), vapor_data, delimiter='\t', header='Temp(K)\tPressure(MPa)\tExperimental(mW/m*K)\tNIST(mW/m*K)\tModel(mW/m*K)')
...
np.savetxt(os.path.join(AdaboostLasso_directory, 'supercritical_data.txt'), supercritical_data, delimiter='\t', header='Temp(K)\tPressure(MPa)\tExperimental(mW/m*K)\tNIST(mW/m*K)\tModel(mW/m*K)')
