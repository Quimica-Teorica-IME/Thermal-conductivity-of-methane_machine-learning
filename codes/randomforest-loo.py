...
Original file is located at
    https://colab.research.google.com/drive/1cpjjh9_X_kJ2rIxQWZvvtx6HI-ZdRrRR
"""

'''
This code uses the Random Forest algorithm. RF is an ensemble learning method
that operates by constructing a multitude of RandomForests at training time
and outputting the class that is the mode of the classes (classification) or
mean prediction (regression) of the individual trees. It builds multiple
RandomForests and merges them together to get a more accurate and stable
prediction.
'''
...
RandomForest_directory = os.path.join(property_directory, "RandomForest-cvloo")
os.makedirs(RandomForest_directory, exist_ok=True)
RandomForest_file = check_and_get_filename(RandomForest_directory, "RandomForest-cvloo", "log")
...
# Grid Search for Hyperparameter Tuning for liquid state with Random Forest
print("Tuning hyperparameters for liquid state with Random Forest...")
...
# Grid Search for Hyperparameter Tuning for vapor state with Random Forest
print("Tuning hyperparameters for vapor state with Random Forest...")
...
save_to_file_and_print(f"\nBest Hyperparameters for vapor state for Random Forest: {grid_search_vapor.best_params_}.\n", RandomForest_file)
...
# Grid Search for Hyperparameter Tuning for supercritical state with Random Forest
print("Tuning hyperparameters for supercritical state with Random Forest...")
...
save_to_file_and_print(f"*** Cross-Validation Metrics ***", RandomForest_file)
...
calculate_metrics("liquid (NIST)", y_liquid_Exp_array, y_liquid_NIST_Expbased_array, file=RandomForest_file)
calculate_metrics("liquid (Model)", y_liquid_Exp_array, y_pred_liquid_cv_array, file=RandomForest_file)

#vapor
save_to_file_and_print(">>> Vapor <<<\n", RandomForest_file)
calculate_metrics("vapor (NIST)", y_vapor_Exp_array, y_vapor_NIST_Expbased_array, file=RandomForest_file)
calculate_metrics("vapor (Model)", y_vapor_Exp_array, y_pred_vapor_cv_array, file=RandomForest_file)

#supercritical
save_to_file_and_print(">>> Supercritical <<<\n", RandomForest_file)
calculate_metrics("supercritical (NIST)", y_supercritical_Exp_array, y_supercritical_NIST_Expbased_array, file=RandomForest_file)
calculate_metrics("supercritical (Model)", y_supercritical_Exp_array, y_pred_supercritical_cv_array, file=RandomForest_file)
...
#liquid
save_to_file_and_print(">>> Liquid <<<\n", RandomForest_file)
calculate_metrics("NIST x Model", y_liquid_NIST_Expbased_array, y_pred_liquid_cv_array, file=RandomForest_file)

#vapor
save_to_file_and_print(">>> Vapor <<<\n", RandomForest_file)
calculate_metrics("vapor (NIST)", y_vapor_NIST_Expbased_array, y_pred_vapor_cv_array, file=RandomForest_file)

#supercritical
save_to_file_and_print(">>> Supercritical <<<\n", RandomForest_file)
calculate_metrics("supercritical (NIST)", y_supercritical_NIST_Expbased_array, y_pred_supercritical_cv_array, file=RandomForest_file)
...
np.savetxt(os.path.join(RandomForest_directory, 'liquid_data.txt'), liquid_data, delimiter='\t', header='Temp(K)\tPressure(MPa)\tExperimental(mW/m*K)\tNIST(mW/m*K)\tModel(mW/m*K)')

# Save vapor data
vapor_data = np.column_stack((X_vapor_Exp, y_vapor_Exp_array, y_vapor_NIST_Expbased_array, y_pred_vapor_cv_array))
np.savetxt(os.path.join(RandomForest_directory, 'vapor_data.txt'), vapor_data, delimiter='\t', header='Temp(K)\tPressure(MPa)\tExperimental(mW/m*K)\tNIST(mW/m*K)\tModel(mW/m*K)')
...
np.savetxt(os.path.join(RandomForest_directory, 'supercritical_data.txt'), supercritical_data, delimiter='\t', header='Temp(K)\tPressure(MPa)\tExperimental(mW/m*K)\tNIST(mW/m*K)\tModel(mW/m*K)')
