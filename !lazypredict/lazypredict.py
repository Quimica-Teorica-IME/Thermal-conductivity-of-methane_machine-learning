...
def remove_outliers(X, y):
    ...
    # Filter the inliers
    X_inliers = X[mask_inliers].tolist()
    y_inliers = y[mask_inliers].tolist()

    # Filter the outliers
    X_outliers = X[~mask_inliers].tolist()
    y_outliers = y[~mask_inliers].tolist()
...
X_liquid_Exp_inliers, y_liquid_Exp_inliers, X_liquid_Exp_outliers, y_liquid_Exp_outliers = remove_outliers(X_liquid_Exp, y_liquid_Exp)
X_vapor_Exp_inliers, y_vapor_Exp_inliers, X_vapor_Exp_outliers, y_vapor_Exp_outliers = remove_outliers(X_vapor_Exp, y_vapor_Exp)
X_supercritical_Exp_inliers, y_supercritical_Exp_inliers, X_supercritical_Exp_outliers, y_supercritical_Exp_outliers = remove_outliers(X_supercritical_Exp, y_supercritical_Exp)


save_to_file_and_print(f"\nData size in liquid phase:\n\tTotal: {len(X_liquid_Exp)}\n\tInliers: {len(X_liquid_Exp_inliers)}\n\tOutliers: {len(X_liquid_Exp_outliers)}", LazyPredict_file)
save_to_file_and_print(f"Data size in vapor phase:\n\tTotal: {len(X_vapor_Exp)}\n\tInliers: {len(X_vapor_Exp_inliers)}\n\tOutliers: {len(X_vapor_Exp_outliers)}", LazyPredict_file)
save_to_file_and_print(f"Data size in supercritical phase:\n\tTotal: {len(X_supercritical_Exp)}\n\tInliers: {len(X_supercritical_Exp_inliers)}\n\tOutliers: {len(X_supercritical_Exp_outliers)}", LazyPredict_file)
...
for x_exp_inlier in X_supercritical_Exp_inliers:
    for i, x_nist in enumerate(X_NIST_Expbased):
        if np.array_equal(x_exp_inlier, x_nist):
...
print(f"Data size in liquid phase (NIST): {len(X_liquid_NIST_Expbased)}, Corresponding experimental data: {len(X_liquid_Exp_inliers)}")
print(f"Data size in vapor phase (NIST): {len(X_vapor_NIST_Expbased)}, Corresponding experimental data: {len(X_vapor_Exp_inliers)}")
print(f"Data size in supercritical phase (NIST): {len(X_supercritical_NIST_Expbased)}, Corresponding experimental data: {len(X_supercritical_Exp_inliers)}")

save_to_file_and_print("\n\n>>> LIQUID PHASE <<<\n", LazyPredict_file)

#           >>>>>       LIQUID PART     <<<<<
# Spliting the data into training (80%) and testing (20%) sets for liquid state
X_liquid_train, X_liquid_test, y_liquid_train, y_liquid_test = train_test_split(X_liquid_Exp_inliers, y_liquid_Exp_inliers, test_size=0.2, random_state=42)
...
X_vapor_train, X_vapor_test, y_vapor_train, y_vapor_test = train_test_split(X_vapor_Exp_inliers, y_vapor_Exp_inliers, test_size=0.2, random_state=42)
...
#           >>>>>       SUPERCRITICAL PART     <<<<<
# Spliting the data into training (80%) and testing (20%) sets for liquid state
X_supercritical_train, X_supercritical_test, y_supercritical_train, y_supercritical_test = train_test_split(X_supercritical_Exp_inliers, y_supercritical_Exp_inliers, test_size=0.2, random_state=42)
