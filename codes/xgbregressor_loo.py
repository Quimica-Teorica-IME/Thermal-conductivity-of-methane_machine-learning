# -*- coding: utf-8 -*-
"""XGBRegressor_loo.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1mSD3UygeX7gGtw4WIcw4o4RHU-mjlv7c
"""
...
XGB_directory = os.path.join(property_directory, "XGB-cvloo")
os.makedirs(XGB_directory, exist_ok=True)
XGB_file = check_and_get_filename(XGB_directory, "XGB-cvloo", "log")
...
# Grid Search for Hyperparameter Tuning for liquid state with XGB
print("Tuning hyperparameters for liquid state with XGB...")
...
# Grid Search for Hyperparameter Tuning for vapor state with XGB
print("Tuning hyperparameters for vapor state with XGB...")
...
# Grid Search for Hyperparameter Tuning for supercritical state with XGB
print("Tuning hyperparameters for supercritical state with XGB...")
...
save_to_file_and_print(f"*** Cross-Validation Metrics ***", XGB_file)
...
calculate_metrics("liquid (NIST)", y_liquid_Exp_array, y_liquid_NIST_Expbased_array, file=XGB_file)
calculate_metrics("liquid (Model)", y_liquid_Exp_array, y_pred_liquid_cv_array, file=XGB_file)

#vapor
save_to_file_and_print(">>> Vapor <<<\n", XGB_file)
calculate_metrics("vapor (NIST)", y_vapor_Exp_array, y_vapor_NIST_Expbased_array, file=XGB_file)
calculate_metrics("vapor (Model)", y_vapor_Exp_array, y_pred_vapor_cv_array, file=XGB_file)

#supercritical
save_to_file_and_print(">>> Supercritical <<<\n", XGB_file)
calculate_metrics("supercritical (NIST)", y_supercritical_Exp_array, y_supercritical_NIST_Expbased_array, file=XGB_file)
calculate_metrics("supercritical (Model)", y_supercritical_Exp_array, y_pred_supercritical_cv_array, file=XGB_file)
...
#liquid
save_to_file_and_print(">>> Liquid <<<\n", XGB_file)
calculate_metrics("NIST x Model", y_liquid_NIST_Expbased_array, y_pred_liquid_cv_array, file=XGB_file)

#vapor
save_to_file_and_print(">>> Vapor <<<\n", XGB_file)
calculate_metrics("vapor (NIST)", y_vapor_NIST_Expbased_array, y_pred_vapor_cv_array, file=XGB_file)

#supercritical
save_to_file_and_print(">>> Supercritical <<<\n", XGB_file)
calculate_metrics("supercritical (NIST)", y_supercritical_NIST_Expbased_array, y_pred_supercritical_cv_array, file=XGB_file)
...
np.savetxt(os.path.join(XGB_directory, 'liquid_data.txt'), liquid_data, delimiter='\t', header='Temp(K)\tPressure(MPa)\tExperimental(mW/m*K)\tNIST(mW/m*K)\tModel(mW/m*K)')

# Save vapor data
vapor_data = np.column_stack((X_vapor_Exp, y_vapor_Exp_array, y_vapor_NIST_Expbased_array, y_pred_vapor_cv_array))
np.savetxt(os.path.join(XGB_directory, 'vapor_data.txt'), vapor_data, delimiter='\t', header='Temp(K)\tPressure(MPa)\tExperimental(mW/m*K)\tNIST(mW/m*K)\tModel(mW/m*K)')
...
np.savetxt(os.path.join(XGB_directory, 'supercritical_data.txt'), supercritical_data, delimiter='\t', header='Temp(K)\tPressure(MPa)\tExperimental(mW/m*K)\tNIST(mW/m*K)\tModel(mW/m*K)')
